...
public protocol CannulaInserter {
    func insertCannula(completion: @escaping (Result<TimeInterval,Error>) -> ())
    func checkCannulaInsertionFinished(completion: @escaping (Error?) -> Void)
}

class OmniBLEPumpManager{
    func insertCannula(completion: @escaping (Result<TimeInterval, Error>) -> ()) {
...
func LocalizedString(_ msg:String, comment:String="comment") -> String{
    return msg
}
    ...
    enum InsertCannulaViewModelState {
        ...
        var actionButtonAccessibilityLabel: String {
            ...
            case .error(let error):
                return String(format: "%@ %@", "error", "error.recoverySuggestion" ?? "")
            case .finished:
        ...
        cannulaInserter.insertCannula { (result) in
            DispatchQueue.main.async {
                ...
                case .success(let finishTime):
                    self.state = .inserting(finishTime:  finishTime)
